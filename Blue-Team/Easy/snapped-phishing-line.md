...
By accessing the phish-emails folder on the Desktop, the email about the Quote for Services Rendered can be investigated. The To field indicates that <mark>William McClean</mark> received the email.
...
In examining the From field, the adversary used <mark>Accounts[.]Payable[@]groupmarketingonline[.]icu</mark> to deliver the phishing emails.
...
To further examine the attachment, it was opened using Firefox. The domain is <mark>kennaroads[.]buzz</mark>.
...
It is impersonating <mark>Microsoft</mark>.
...
By only navigating to this (hxxp[://]kennaroads[.]buzz/data/) path, the <mark>Update365zip</mark> can be located.
...
The hash of Update365 is <mark>ba3c15267393419eb08c7b2652b8b6b39b406ef300ae8a18fee4d16b19ac9686</mark>. This was achieved by using the command:
...
It is also listed as a <mark>Trojan</mark>.
...
In the Details tab, under Bundle Info, its metadata says that it contains <mark>49</mark> files.
...
The log.txt file shows that Micahel Ascot submitted his credentials twice. His email address is <mark>michael[.]ascot@swiftspend[.]finance</mark>.
...
After extracting the ZIP file, the output was piped to grep to locate for the <mark>submit.php</mark> file. This was achieved by using the command:
...
<mark>m3npat[@]yandex[.]com</mark> was the email address that the attacker was using to collect compromised credentials.

### 11. Return to the phishing URL and locate the *flag.txt* file. Using CyberChef to decode the flag, what is the secret value?
By utilizing the Hint button from the Room, the Flag was located from this (hxxp[://]kennaroads[.]buzz/data/Update365/office365/flag[.]txt) path. However the Flag is Base64 encoded, CyberChef can decode this.
...
The Flag is <mark>THM{pL4y_w1Th_tH3_URL}</mark>.
